...
let failed = false;
try {
  for (const preferred of ['auto', 'webgl2', 'classic']) {
    for (const shift of ['', '1']) {
      const page = await browser.newPage({ viewport: { width: 960, height: 540 } });
      const logs = [];
      page.on('console', (m) => { if (m.type() === 'error' || m.type() === 'warning') logs.push(`${m.type()}: ${m.text()}`); });
      page.on('pageerror', (e) => logs.push(`pageerror: ${e.message}`));
      const q = preferred === 'classic' ? 'preferred=auto&classic=1' : `preferred=${preferred}`;
      await page.goto(`${url}tests/render/harness/index.html?${q}&shift=${shift}`);
