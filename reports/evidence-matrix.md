...
| Status | Count | IDs |
| --- | --- | --- |
| PASS | 9 | P02, P03, P05, W01, W04, G02, G04, G05, G06 |
| PARTIAL | 11 | P01, P04, R03, R04, W02, W03, G01, G03, O01, O02, O03 |
| FAIL | 1 | R05 |
...
| G01 | Anchor transitions collision safe | **PARTIAL** | Unit: `tests/sim/gravity.test.ts` covers every anchor, both directions, blocked landings and out-of-volume requests, all PASS. Browser: the real WASD+F shift commits (`real-input-shift.json`: `ShiftCommitted`, up `+y`→`-z`). A request that reaches the sim outside an anchor is rejected with state intact. Failure 2 (real input never makes the request) is a spec mismatch on the failure trace. | W4/W1 (VR6). |
